...
vertices = data_frame(
	name = c("S10","S12","S11"),
	x0 = c(+SENSOR_DIST/2, SENSOR_DIST, 0),
	y0 = c(0, +HEIGHT_LEN, +HEIGHT_LEN),
	ang.right = c(to.rad(60), to.rad(180), to.rad(360-60)),
...
edges = vertices %>%
	select("name"=o.name, "o.name"=name, x0, y0) %>%
	mutate(
		x1 = c(+SENSOR_DIST, 0, +SENSOR_DIST/2),
		y1 = c(+HEIGHT_LEN, +HEIGHT_LEN, 0)
	)
